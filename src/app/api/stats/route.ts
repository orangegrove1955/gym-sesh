...
function computeStats(
  ...
  if (sortedDates.length > 0) {
    const startDate =
      sortedDates[0] === today || sortedDates[0] === yesterday ? 0 : -1;
    if (startDate >= 0) {
  ...
  const weeksActive = firstWorkout
    ? Math.max(
        1,
        differenceInCalendarWeeks(new Date(), new Date(firstWorkout)) + 1
      )
    : 1;
  ...
  const weeklyVolume = Object.entries(weeklyVolumeMap).map(
    ([week, volume]) => ({
      week,
  ...
  const muscleVolume = Object.entries(muscleWeeklyMap).map(
    ([week, muscles]) => ({
      week,
      ...Object.fromEntries(
...
export async function GET() {
  ...
  const [sessionsRes, setLogsRes, exercisesRes] = await Promise.all([
    supabase
    ...
    supabase.from("set_logs").select("*").eq("completed", true),
